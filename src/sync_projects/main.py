...
def link_to_bioproject(db: Connection, entrez: EntrezAdvanced, linked_object: str):
    log.info(
        "Starting the bioprojects synchronization for %s objects",
        linked_object,
    )
    ...
    total_count = sql.get_object_without_project_imported_count(db, tmp_package_id, linked_object)
    ...
    while True:
        ...
        object = sql.get_objects_without_project_imported(
            db, tmp_package_id, per_page=entrez.DEFAULT_PER_PAGE, last_id=last_id, obj=linked_object
        )
        if not object:
            break
        ...
        if linked_object == "samplealias":
            biosample_ids : list[int] = [
                int(id)
                for result in entrez.esearch(DB.BIO_SAMPLE, " OR ".join([aliases[1] + "[ACCESSION]" for aliases in object]))
                for id in result[0]
            ]

            # missing_samples = len(object_bioproject_matches) - len(biosample_ids)

            # if missing_samples:
            #     log.info(
            #         "[Page %s/%s] %s aliases(s) were not returned by the BioSample API.",
            #         page_num,
            #         pages_total,
            #         missing_samples,
            #     )
            ...
            for biosample_xml in biosamples_xml:
                try:
                    initial_aliases_gathered[int(biosample_xml.attrib["id"])] = '%'+biosample_xml.attrib["accession"]+'%'
                except Exception:
        ...
        object_bioproject_matches = entrez.elink(
            DB.BIO_SAMPLE, DB.BIO_PROJECT,
            list(initial_aliases_gathered.keys()) if initial_aliases_gathered else [sample[1] for sample in object],
        ...
        for object_id, project_id in object_bioproject_matches.items():
            if project_id not in [
                ...
                bioproject_object_reversed.setdefault(project_id, []).append(initial_aliases_gathered.get(object_id, object_id))

        #Efetch on the bioproject to get full name, for creation
        projects = entrez.get_projects(set(bioproject_object_reversed.keys()))
        ...
        for project in projects:
            ...
            db_package_id = sql.get_or_create_data_package_by_ncbi_id(db, project.id, f"[{project.name}] {project.title}")
