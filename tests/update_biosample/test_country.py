...
from src.sync_samples.extractors import get_country
...
def test_country():
    for variant in [
        ("China:Shanghai", ["CHN", "Shanghai"]),
        ("France", ["FRA", None]),
        (None, [None, None]),
        ("NA", [None, None]),
        ("St Margaret's Hospital", ["ZAF", "St Margaret's Hospital"]),
    ]:
        test_payload = BIOSAMPLE_CONTENT_XML.format(country=variant[0])
        elements = ET.fromstring(test_payload)
        code, extra = get_country(elements[0])
        assert code == variant[1][0], f"Variant '{variant[0]}' has failed"
        assert extra == variant[1][1], f"Variant '{variant[0]}' has failed"
