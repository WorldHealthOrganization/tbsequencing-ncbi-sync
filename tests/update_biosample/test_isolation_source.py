...
from src.sync_samples.selectors import get_isolation_source
    ...
    <BioSample access="public" publication_date="2022-10-05T00:50:11.397" last_update="2022-10-05T00:50:11.397"
        ...
        <Attributes>
            ...
            <Attribute attribute_name="isolation_source" harmonized_name="isolation_source"
                       display_name="isolation source">{isolation_source}
            </Attribute>
...
def test_isolation_source():
    for variant in [("expectoration S3", "expectoration S3"), (None, "")]:
        test_payload = BIOSAMPLE_CONTENT_XML.format(isolation_source=variant[0])
        elements = ET.fromstring(test_payload)
        source = get_isolation_source(elements[0])
        assert source == variant[1], f"Variant '{variant[0]}' has failed"
