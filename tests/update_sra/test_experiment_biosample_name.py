...
from src.sync_sequencing_data.selectors import guess_experiment_biosample_name, guess_experiment_run_biosample_name
                    ...
                    <EXPERIMENT_PACKAGE_SET>\n
                        <EXPERIMENT_PACKAGE>
                            <EXPERIMENT accession="SRX18158176" alias="QC_2020_nr06_filt_R1">
                                ...
                                <DESIGN>
                                    ...
                                    <LIBRARY_DESCRIPTOR>
                                        ...
                                        <LIBRARY_SOURCE>b</LIBRARY_SOURCE>
                                        <LIBRARY_SELECTION>a</LIBRARY_SELECTION>
                                        <LIBRARY_LAYOUT>
                                            <OUT/>
                                        </LIBRARY_LAYOUT>
                                    </LIBRARY_DESCRIPTOR>
                            ...
                            <SAMPLE alias="QC_2020_nr06" accession="SRS15658349">
                                <IDENTIFIERS>
                                    <PRIMARY_ID>SRS15658349</PRIMARY_ID>
                                    <EXTERNAL_ID namespace="BioSample">{biosample}</EXTERNAL_ID>
...
def test_experiment_biosample_name():
    for variant in [("SAMN31551447", "SAMN31551447")]:
        test_payload = SRA_CONTENT_XML.format(biosample=variant[0])
        elements = ET.fromstring(test_payload)
        biosample = guess_experiment_biosample_name(elements[0])
        assert biosample == variant[1], f"Variant '{variant[0][0]}' has failed"


def test_experiment_run_biosample_name():  # TODO with not none variants
    test_payload = SRA_CONTENT_XML.format(biosample=None)
    elements = ET.fromstring(test_payload)
    with pytest.raises(Exception):
        guess_experiment_run_biosample_name(elements[0])
